...
import { Target, ChevronRight, ChevronLeft, Calendar } from "lucide-react";

const STATUS_CONFIG = {
    identified: { label: "Identified", barClass: "bg-gray-500" },
    contacted: { label: "Contacted", barClass: "bg-blue-500" },
    meeting_scheduled: { label: "Meeting", barClass: "bg-amber-500" },
    not_interested: { label: "Not Interested", barClass: "bg-rose-500" },
};
...
export default function ProspectsTimelinePage() {
    ...
    const [selectedItem, setSelectedItem] = useState<ProspectItem | null>(null);
        ...
        const fetchData = async () => {
            ...
            try {
                const { data, error } = await supabase
                    ...
                    .select('id, company_name, industry, status, start_date, end_date, progress')
    ...
    const { timelineStart, timelineEnd, totalDays, markers } = useMemo(() => {
        ...
        let minDate = parseDate(prospectsData[0].start_date);
        let maxDate = parseDate(prospectsData[0].end_date);
    ...
    }, [prospectsData, viewMode]);

    const getBarStyle = (item: ProspectItem) => {
        ...
        <div className="flex flex-col h-full">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <div className="flex items-center gap-4">
                    <Link href="/dashboard/bisdev/prospects" className="p-2 rounded-lg border border-[var(--glass-border)] hover:bg-[var(--glass-bg-hover)]"><ChevronLeft className="h-5 w-5 text-[var(--text-secondary)]" /></Link>
                    <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-cyan-500 to-cyan-600 flex items-center justify-center shadow-lg shadow-cyan-500/20"><Target className="h-6 w-6 text-white" /></div>
                    <div>
                        <div className="flex items-center gap-2 mb-1 text-sm text-[var(--text-secondary)]">
                            ...
                            <Link href="/dashboard/bisdev/prospects" className="hover:text-[var(--text-primary)]">Prospects</Link><ChevronRight className="h-4 w-4" />
                            <span className="text-[var(--text-primary)]">Timeline</span>
                        </div>
                        <h1 className="text-2xl font-bold text-[var(--text-primary)]">Prospects Timeline</h1>
                        <p className="text-sm text-[var(--text-secondary)]">Gantt chart visualization of prospects</p>
                    </div>
                </div>
            ...
            <div className="flex-1 rounded-2xl border border-[var(--glass-border)] bg-white dark:bg-[#1c2120] overflow-hidden">
                {prospectsData.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-64"><Calendar className="h-12 w-12 text-[var(--text-muted)] mb-4" /><p className="text-[var(--text-secondary)]">No prospects data with dates</p></div>
                ) : (
                    <div className="overflow-x-auto">
                        <div className="sticky top-0 z-10 bg-white dark:bg-[#1c2120] border-b border-[var(--glass-border)]">
                            <div className="flex">
                                <div className="w-64 min-w-64 p-4 border-r border-[var(--glass-border)] bg-black/5 dark:bg-white/5"><span className="text-xs font-bold text-[var(--text-muted)] uppercase">Prospect</span></div>
                            ...
                            {prospectsData.map((item) => (
                                <div key={item.id} className="flex border-b border-[var(--glass-border)] hover:bg-black/5 dark:hover:bg-white/5">
                                    <div className="w-64 min-w-64 p-4 border-r border-[var(--glass-border)]">
                                        ...
                                        <p className="text-xs text-[var(--text-secondary)] truncate">{item.industry || 'No industry'}</p>
                                    </div>
            ...
            {selectedItem && (
                <div className="mt-4 p-4 rounded-2xl border border-[var(--glass-border)] bg-white dark:bg-[#1c2120]">
                    <div className="flex items-start justify-between"><div><h3 className="font-bold text-lg text-[var(--text-primary)]">{selectedItem.company_name}</h3><p className="text-sm text-[var(--text-secondary)]">{selectedItem.industry || 'No industry'}</p></div><button onClick={() => setSelectedItem(null)} className="p-2 rounded-lg hover:bg-black/10">×</button></div>
            ...
            <div className="mt-4 flex flex-wrap gap-4">{(Object.keys(STATUS_CONFIG) as ProspectStatus[]).map((s) => <div key={s} className="flex items-center gap-2"><div className={`w-4 h-4 rounded ${STATUS_CONFIG[s].barClass}`}></div><span className="text-xs text-[var(--text-secondary)]">{STATUS_CONFIG[s].label}</span></div>)}</div>
        </div>
    );
}
