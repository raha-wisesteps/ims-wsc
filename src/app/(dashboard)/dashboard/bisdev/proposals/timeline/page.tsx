...
const STATUS_CONFIG = {
    draft: { label: "Draft", barClass: "bg-gray-500" },
    sent: { label: "Sent", barClass: "bg-blue-500" },
    negotiation: { label: "Negotiation", barClass: "bg-amber-500" },
    approved: { label: "Approved", barClass: "bg-emerald-500" },
    rejected: { label: "Rejected", barClass: "bg-rose-500" },
};
...
export default function ProposalsTimelinePage() {
        ...
        const fetchData = async () => {
            ...
            try {
                const { data, error } = await supabase
                    ...
                    .select('id, proposal_title, company_name, status, start_date, end_date, progress, offer_value')
    ...
    const { timelineStart, timelineEnd, totalDays, markers } = useMemo(() => {
        ...
        let minDate = parseDate(proposalsData[0].start_date);
        let maxDate = parseDate(proposalsData[0].end_date);
    ...
    }, [proposalsData, viewMode]);

    const getBarStyle = (item: ProposalItem) => {
        ...
        <div className="flex flex-col h-full">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <div className="flex items-center gap-4">
                    <Link href="/dashboard/bisdev/proposals" className="p-2 rounded-lg border border-[var(--glass-border)] hover:bg-[var(--glass-bg-hover)]"><ChevronLeft className="h-5 w-5 text-[var(--text-secondary)]" /></Link>
                    <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center shadow-lg shadow-purple-500/20"><FileText className="h-6 w-6 text-white" /></div>
                    <div>
                        <div className="flex items-center gap-2 mb-1 text-sm text-[var(--text-secondary)]">
                            ...
                            <Link href="/dashboard/bisdev/proposals" className="hover:text-[var(--text-primary)]">Proposals</Link><ChevronRight className="h-4 w-4" />
                            <span className="text-[var(--text-primary)]">Timeline</span>
                        </div>
                        <h1 className="text-2xl font-bold text-[var(--text-primary)]">Proposals Timeline</h1>
                        <p className="text-sm text-[var(--text-secondary)]">Gantt chart visualization of proposals</p>
                    </div>
                </div>
            ...
            <div className="flex-1 rounded-2xl border border-[var(--glass-border)] bg-white dark:bg-[#1c2120] overflow-hidden">
                {proposalsData.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-64"><Calendar className="h-12 w-12 text-[var(--text-muted)] mb-4" /><p className="text-[var(--text-secondary)]">No proposals data with dates</p></div>
                ) : (
                    <div className="overflow-x-auto">
                        <div className="sticky top-0 z-10 bg-white dark:bg-[#1c2120] border-b border-[var(--glass-border)]">
                            <div className="flex">
                                <div className="w-64 min-w-64 p-4 border-r border-[var(--glass-border)] bg-black/5 dark:bg-white/5"><span className="text-xs font-bold text-[var(--text-muted)] uppercase">Proposal</span></div>
                            ...
                            {proposalsData.map((item) => (
                                <div key={item.id} className="flex border-b border-[var(--glass-border)] hover:bg-black/5 dark:hover:bg-white/5">
                                    <div className="w-64 min-w-64 p-4 border-r border-[var(--glass-border)]">
                                        <h4 className="font-bold text-sm text-[var(--text-primary)] truncate">{item.proposal_title}</h4>
            ...
            {selectedItem && (
                <div className="mt-4 p-4 rounded-2xl border border-[var(--glass-border)] bg-white dark:bg-[#1c2120]">
                    <div className="flex items-start justify-between"><div><h3 className="font-bold text-lg text-[var(--text-primary)]">{selectedItem.proposal_title}</h3><p className="text-sm text-[var(--text-secondary)]">{selectedItem.company_name}</p></div><button onClick={() => setSelectedItem(null)} className="p-2 rounded-lg hover:bg-black/10">×</button></div>
                    <div className="grid grid-cols-4 gap-4 mt-4">
                        ...
                        <div><span className="text-xs text-[var(--text-muted)]">Offer Value</span><p className="text-sm font-medium text-[#e8c559]">{formatCurrency(selectedItem.offer_value)}</p></div>
            ...
            <div className="mt-4 flex flex-wrap gap-4">{(Object.keys(STATUS_CONFIG) as ProposalStatus[]).map((s) => <div key={s} className="flex items-center gap-2"><div className={`w-4 h-4 rounded ${STATUS_CONFIG[s].barClass}`}></div><span className="text-xs text-[var(--text-secondary)]">{STATUS_CONFIG[s].label}</span></div>)}</div>
        </div>
    );
}
